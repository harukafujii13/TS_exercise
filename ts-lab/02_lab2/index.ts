...
const ExerciseTwo = () => {
  ...
  function addToCart(item: CertItem){
    console.log('[Exercise 2.1]', `Adding "${item.title}" to cart.`)
  }


  addToCart({ id: 1, title: 'Concrete shoes', varianrId: 1010});
  ...
  class Person implements Charactor{
  ...
  // user.id = 5 // not writable　//プロパティを書き換えることはできない！
...
const Exercise3 = () => {
  ...
  const bankAccount = {
      money: 0,
      deposit(value: number, message?: string) { //coercion to a boolean value
          this.money += value
          if (message) {
              console.log(message)
          }
  ...
  const defaultGreeting = greet('hello').toLocaleLowerCase();
